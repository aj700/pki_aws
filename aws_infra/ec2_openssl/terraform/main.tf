...
resource "aws_security_group" "pki_server" {
  name        = "acme-pki-server-sg"
  description = "Security group for PKI server"
  ...
  # HTTP for CRL/OCSP (optional)
  ingress {
    from_port   = 80
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
    description = "HTTP for CRL distribution"
  }
...
# SSM policy for Session Manager access (optional, more secure than SSH)
